...
class LugarModel extends Mysql
{
    //Propiedades
    private $intIdLugar;
    private $strNombre;
    ...
    {
        $sql = "SELECT idLugar,nombre,descripcion,status, CONCAT(DAY(dateCreate), ' de ', 
            ...
            END, ' del ', YEAR(dateCreate)) AS fechaCreacion,DATE_FORMAT(dateCreate, '%h:%i:%s %p') AS horaCreacion FROM lugarqueja WHERE status != 0";
        $request = $this->select_All($sql);
        return $request;
    }

    //Metodo para insertar lugares de queja a la base 
    public function insertLugar(string $nombre, string $descripcion, int $status)
    {
        ...
        $sql = "SELECT * FROM lugarqueja WHERE nombre = '{$this->strNombre}'";
        ...
        if (empty($request)) {
            ...
            $query_insert = "INSERT INTO lugarqueja(nombre,descripcion,status,dateCreate) VALUES (?,?,?,DATE_SUB(NOW(), INTERVAL 5 HOUR))";
    ...
    //Metodo para extraer la informacion de la queja de lugar
    public function selectLugar(int $idLugar)
    {
        ...
        $sql = "SELECT idLugar, nombre, descripcion, status FROM lugarqueja WHERE idLugar = $this->intIdLugar";
        $request = $this->select($sql);
        return $request;
    }

    //Metodo para actualizar la informacion de la queja de lugar 
    public function updateLugar(int $idLugar, string $nombre, string $descripcion, int $status)
    {
        //Asignamos los valores de los parametos a las propiedades 
        $this->intIdLugar = $idLugar;
        $this->strNombre = $nombre;
        ...
        $sql = "SELECT * FROM lugarqueja WHERE (nombre = '{$this->strNombre}' AND idLugar != $this->intIdLugar)";
        $request = $this->select_All($sql);
        if (empty($request)) {
            //creamos la consulta ya para actualizar el registro en la base
            $sql = "UPDATE lugarqueja SET nombre = ?, descripcion = ?, status = ? WHERE idLugar = $this->intIdLugar";
            $arrData = array($this->strNombre, $this->strDescripcion, $this->intStatus);
    ...
    //Metodo para eliminar lugares de quejas
    public function deleteLugares(int $idLugar)
    {
        // Asignamos el valor del parámetro a la propiedad
        $this->intIdLugar = $idLugar;
        // Consulta SQL para actualizar el estado del rol
        $sql = "UPDATE lugarqueja SET status = ? WHERE idLugar = ?";
        // Arreglo de datos para la consulta preparada
        $arrData = array(0, $this->intIdLugar);
