...
function fntRegistrosGirs() {
  tableGirs = $("#table_girs").DataTable({
    ...
    columns: [
      ...
      { data: "compensation", className: "text-center" },
...
function fntOpcionesSelect() {
  ...
  request.onreadystatechange = function () {
    if (request.readyState === 4 && request.status === 200) {
      ...
      document.querySelector("#listQueja").innerHTML =
        '<option value="" selected disabled style="color:#800000;">⚠ Select an Opportunity</option>' +
  ...
  request.onreadystatechange = function () {
    if (request.readyState == 4 && request.status == 200) {
      document.querySelector("#listLugar").innerHTML =
        '<option value="" selected disabled style="color:#800000;">⚠ Select a Place Of Opportunity </option>' +
        request.responseText;

      document.querySelector("#listLugarUpdate").innerHTML = request.responseText;
...
function fntDepartamentos() {
  ...
  request.onreadystatechange = function () {
    if (request.readyState == 4 && request.status == 200) {
      ...
      document.querySelector("#listDepartamentoUpdate").innerHTML = request.responseText;
...
//Function para extraer las compensacion
function fntCompensation() {
  let request = window.XMLHttpRequest
    ? new XMLHttpRequest()
    : new ActiveXObject("Microsoft.XMLHTTP");

  let ajaxUrl = Base_URL + "/Girs/getSelectCompensations";
  request.open("GET", ajaxUrl, true);
  request.send();

  request.onreadystatechange = function () {
    if (request.readyState == 4 && request.status == 200) {
      document.querySelector("#listCompensation").innerHTML =
        '<option value="" selected disabled style="color:#800000;">⚠ Select a compensation </option>' +
        request.responseText;

      document.querySelector("#listCompensationUpdate").innerHTML = request.responseText;
    }
  };
}
...
function fntAgregarGirs() {
  ...
  btnGirs.addEventListener("click", () => {
    ...
    formGirs.onsubmit = (e) => {
      ...
      const intCompensation = document.querySelector("#listCompensation");
      ...
      if (
        intCompensation.value === "" ||
        strFecha.value === "" ||
        strApellidos.value.trim() === "" ||
        intVilla.value === "" ||
        strEntrada.value === "" ||
        strSalida.value === "" ||
        intEstado.value === "" ||
        ...
        strDescripcion.value.trim() === "" ||
        strAccion.value.trim() === "" ||
        strSeguimiento.value.trim() === ""
      ) {
        Swal.fire({
          title: "¡Attention!",
...
function btnUpdateGir(idGir) {
  document.getElementById("idGir").value = idGir;
    const btnGirs = document.querySelector("#btn");
  ...
  fetch(Base_URL + "/Girs/getSelectQuejas")
    ...
    .then((objData) => {
      ...
      // Llenar campos
      document.querySelector("#listCompensationUpdate").value =
        data.id_compensation;
      ...
      document.querySelector("#listQuejaUpdate").value = data.idQueja;
      ...
      document.querySelector("#imagen-previewU").src =
        `${Media}/Imagenes_almacenadas/${data.imagen}`;
